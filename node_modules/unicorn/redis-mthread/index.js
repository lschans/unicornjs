...
module.exports = function (config, redis) {
   var cluster = require('cluster'),
       masterObj = require(Path.join(config.rootdir, 'unicorn_services', config.process.name, config.process.masterCode))(config, redis, unicorn),
       workerObj = require(Path.join(config.rootdir, 'unicorn_services', config.process.name, config.process.workerCode))(config, redis, unicorn);
   ...
   if (cluster.isMaster) { // Master state, this code will be executed by the master process
       ...
       // This function distributes the load over the workers
       ...
       //--------------------------------------------------------------------------------------------------------------------------------
       // This part makes it the redis-mthread a message comes in over redis and is processed
       // If we can make the rest of the module generic, so the message comes in and....
       // then the message to thread can be from anywhere, net socket, unix socket, etc

       businit.listenBroker(config, listenClient, publishClient).then(function (Channel) {

           console.log('BROKER This is the assign channel '+ Channel);

           businit.getChannel(config, Channel).then(function (message) {

               var msg = JSON.parse(message);
               console.log('BROKER %s', msg.serviceID);
           ...
           // Test if the message is a service message for the service itself, e.g. restart, more threads, getThreads etc.
           //
           //businit.getChannel(config, Channel).then(balanceMessage(message));
       });

       // End of the redis specific part
       //--------------------------------------------------------------------------------------------------------------------------------
