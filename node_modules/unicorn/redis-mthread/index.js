...
module.exports = function (config, redis, masterObjMod, workerObjMod) {
   var config = xtend(defaultConfig, config),
       cluster = require('cluster'),
       masterObj = require(Path.join(config.rootdir, masterObjMod)),
       workerObj = require(Path.join(config.rootdir, workerObjMod));
   ...
   if (cluster.isMaster) { // Master state, this code will be executed by the master process
       ...
       listenClient.subscribe(config.redis.channel);

       //Execute injected start code
       if(typeof(masterObj.startLogic) == 'function') masterObj.startLogic();
       ...
       console.log('Master cluster setting up ' + config.cluster.threads + ' workers...');
       ...
       var dieIn = Math.round(Math.random() * (config.service.dieMax - config.service.dieMin) + config.service.dieMin);
       setTimeout(function() {
       ...
       //Execute injected start code
       if(typeof(workerObj.startLogic) == 'function') workerObj.startLogic();
