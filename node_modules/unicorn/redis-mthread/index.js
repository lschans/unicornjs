...
*/
       ...
       // Asking to the broker for a channel to subscribe
       businit.listenBroker(config, listenClient, publishClient).then(function (channel) {
           ...
           var internalPublishChannel = redis.createClient(config.redis.port, config.redis.server, {});
           ...
           internalListenChannel.on('message', function (channel, message) {
               ...
               if (!busmsg.verify(msg)) {
                   ...
                   console.log('I am '+process.pid+' So, this broker has a message to transmit');
                   //Broker work forward this message ....
                   //check services with this name
                   if(pids.length > 0) { // Check if we have workers to balance to
                       if (rrCounter > (pids.length - 1)) rrCounter = 0;
                       // Forward the message to the next available worker
                       workers[pids[rrCounter++]].send({pid:process.pid, type:'MSG', msg:message});
                   ...
                   //internalPublishChannel.publish(msg.respondChannel, JSON.stringify(msg));
               }
           });

           if (channel.search('broker') != -1){
               ...
               brokerClient.on('message', function (channel, message) {
                   ...
                   if (!busmsg.verify(msg)) {
                       ...
                       console.log("somebody publish in the broker channel");
                       //Particular logic of the service... that's why this shoud be a internal module in each service
                       //internalPublishChannel.publish(msg.respondChannel, JSON.stringify(msg));
                   }
               });
