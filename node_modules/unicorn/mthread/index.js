...
module.exports = function (config, redis) {
    var cluster = require('cluster'),
        masterObj = require(Path.join(config.rootdir, 'unicorn_services', config.process.name, config.process.masterCode))(config, redis, msgHook),
        workerObj = require(Path.join(config.rootdir, 'unicorn_services', config.process.name, config.process.workerCode))(config, redis);

    // Hook to insert messages into the system that are not from redis, use this for sockets etc.
    function msgHook(message) {
        var newMessage = {
            msg:message,
            pid:process.pid
        };
        balanceMessage(newMessage);
    }
    ...
    if (cluster.isMaster) { // Master state, this code will be executed by the master process
        var listenClient = redis.createClient(config.redis.port, config.redis.server, {}),
            publishClient = redis.createClient(config.redis.port, config.redis.server, {}),c
        ...
        function balanceMessage(message) {
            if (pids.length > 0) { // Check if we have workers to balance to
                if (rrCounter >= pids.length) rrCounter = 0;

                // Forward the message to the next available worker
                var toThread = pids[rrCounter++];

                if(toThread !== undefined) { // If the rage condition if there that the array pos is empty, rebalance the message
                    console.log('available: ' + pids.join(',') + ' Sending message to: ' + toThread, message);
                    workers[toThread].send({pid: process.pid, type: 'MSG', msg: message});
                } else {
